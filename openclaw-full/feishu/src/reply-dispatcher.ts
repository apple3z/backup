...
export function createFeishuReplyDispatcher(params: CreateFeishuReplyDispatcherParams) {
  ...
  const debugMode = account.config?.debugMode ?? false;
  ...
  const { dispatcher, replyOptions, markDispatchIdle } =
    core.channel.reply.createReplyDispatcherWithTyping({
      ...
      deliver: async (payload: ReplyPayload, info) => {
        // Extract thinking/reasoning content from payload
        const thinking = (payload as any).thinking ?? (payload as any).reasoning ?? null;
        const maxThinkingLength = 3000;
        
        // Get message type for labeling
        const msgType = info?.kind === "block" ? "🔧 Tool" : (info?.kind === "final" ? "🤖 Assistant" : "💬");
        ...
        if (hasText) {
          ...
          if (useCard) {
            ...
            // Prepend thinking chain if present and this is final message
            let finalText = converted;
            if (thinking && info?.kind === "final") {
              const truncatedThinking = thinking.length > maxThinkingLength
                ? thinking.slice(0, maxThinkingLength) + "\n...（思维链过长已截断）"
                : thinking;
              finalText = `💭 **思维链**\n\`\`\`\n${truncatedThinking}\n\`\`\`\n\n${msgType} ${converted}`;
            } else {
              finalText = `${msgType} ${converted}`;
            }

            // Add debug info if debugMode is enabled
            let debugText = "";
            if (debugMode && info?.kind === "final") {
              const debugInfo = {
                messageId: payload.replyToId || "N/A",
                chatId: chatId,
